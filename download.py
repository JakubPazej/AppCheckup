...
#Made to fake and rotate agent lists to get around download restrictions
user_agent_list = [
...
def main():
    ...
    print('%sDownload script starting up%s' %
    ...
    g_play_res = requests.get(google_play_url + package_id,
    ...
    try:
        ...
        print("%sKey Error! you most likely reached the download limit on apksfull.com:%s" %
             (fg('red'), attr('reset')))
        print(dl_res_json)
        quit()
    ...
    app_name = package_id.replace("com.", "")
    ...
    print('%sAPK Downloaded%s' %
          (fg('light_green'), attr('reset')))

    tmp="%sFile located in output/" +app_name+".apk%s"
    print( tmp %
          (fg('green'), attr('reset')))
